...
## 角度单位
- `sin` / `cos` / `tan` 的参数默认按「度」解释；需要弧度时在表达式后加 `r` 后缀，如 `sin(\x r)`、`sin(0.5*pi r)`。
- 反三角 `asin` / `acos` / `atan` / `atan2` 的返回值单位是「度」；需要弧度时乘 `pi/180` 或用 `rad()` 转换，如 `atan(\x)*pi/180`。
- `rad(x)` 把度转成弧度，`deg(x)` 把弧度转成度。
